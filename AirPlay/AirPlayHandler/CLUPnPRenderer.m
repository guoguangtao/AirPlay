...
- (void)seek:(float)relTime{
    [self seekToTarget:[NSString stringWithDurationTime:relTime] Unit:@"1"];
}
...
- (void)postRequestWith:(CLUPnPAction *)action{
    ...
    NSURL *url = [NSURL URLWithString:[action getInPostUrlStrWith:_model]];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    request.HTTPMethod = @"POST";
    [request addValue:@"text/xml" forHTTPHeaderField:@"Content-Type"];
    [request addValue:[action getAboutSOAPAction] forHTTPHeaderField:@"SOAPAction"];
    request.HTTPBody = [[action getInPostXMLFile] dataUsingEncoding:NSUTF8StringEncoding];
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error || data == nil) {
//            CLLog(@"error=%@", error);
            [self _UndefinedResponse:[action getInPostXMLFile]];
            return;
        }else{
